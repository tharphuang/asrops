...
class RequestApi(object):
    ...
    def gene_params(self, apiname, taskid=None, slice_id=None):
        ...
        if apiname == api_prepare:
            ...
            param_dict['speaker_number'] = str(0)
            param_dict['has_seperate'] = "true"
            param_dict['role_type'] = str(1)
    ...
    def gene_request(self, apiname, data, files=None, headers=None):
        ...
        if result["ok"] == 0:
            ...
            if apiname == api_get_result:
                datas = json.loads(result["data"])
                datas.sort(key=itemgetter('speaker'))

                print(datas)
                with open("/var/www/html/codehub/right.txt",'a') as f:
                    for k, items in groupby(datas, key=itemgetter('speaker')):
                        print(k)
                        f.write(str(k)+'\n')
                        for i in items:
                            print(' ', i)
...
if __name__ == '__main__':
    api = RequestApi(appid="5eec1eb2", secret_key="8fe26f46d95ba0dad2fbd485b2445936", upload_file_path=r"/var/www/html/codehub/right.wav")
    api.all_api_request()